...
export async function buildInvoicePdf(input: InvoicePdfInput) {
  ...
  const estimateTextWidth = (text: string, size: number) => text.length * size * 0.5;

  const drawText = (x: number, top: number, size: number, text: string, bold = false) => {
    push(`BT /${bold ? 'F2' : 'F1'} ${size} Tf 1 0 0 1 ${x} ${toPdfY(top)} Tm (${escapePdfText(text)}) Tj ET`);
  };

  const drawRightText = (right: number, top: number, size: number, text: string, bold = false) => {
    const width = estimateTextWidth(text, size);
    drawText(right - width, top, size, text, bold);
  };

  const drawCenteredText = (left: number, top: number, width: number, size: number, text: string, bold = false) => {
    const textWidth = estimateTextWidth(text, size);
    drawText(left + (width - textWidth) / 2, top, size, text, bold);
  };

  const drawLine = (x1: number, top1: number, x2: number, top2: number) => {
    push(`${x1} ${toPdfY(top1)} m ${x2} ${toPdfY(top2)} l S`);
  };

  const drawRect = (x: number, top: number, width: number, height: number, fill = false) => {
    push(`${x} ${PAGE_HEIGHT - top - height} ${width} ${height} re ${fill ? 'B' : 'S'}`);
  };

  const drawImage = (name: string, x: number, top: number, width: number, height: number) => {
    push(`q ${width} 0 0 ${height} ${x} ${PAGE_HEIGHT - top - height} cm /${name} Do Q`);
  };

  const setStrokeGray = (gray: number) => {
    push(`${gray} G`);
  };

  const setFillGray = (gray: number) => {
    push(`${gray} g`);
  };

  const setStrokeRgb = (red: number, green: number, blue: number) => {
    push(`${red} ${green} ${blue} RG`);
  };

  const setFillRgb = (red: number, green: number, blue: number) => {
    push(`${red} ${green} ${blue} rg`);
  };

  const drawCircle = (centerX: number, centerY: number, radius: number, fill = false) => {
  ...
  const drawBrandLogo = (left: number, top: number) => {
    const logoBlue: [number, number, number] = [0.086, 0.435, 0.898];
  ...
  const drawSection = (top: number, title: string, bodyHeight: number) => {
  ...
  const rows = [
    ['Frame', currency(input.framePrice)],
    ['Lens', currency(input.lensPrice)],
    ['Subtotal', currency(input.framePrice + input.lensPrice)],
    ['Discount', `- ${currency(input.discount)}`],
    ['Total Amount', currency(input.totalPayable)],
    ['Paid Now', currency(input.paidAmount)],
    ['Remaining Amount', currency(input.remainingAmount)],
  ];
  ...
  drawRightText(PAGE_MARGIN + CONTENT_WIDTH, 74, 10.5, `Order ID: ${input.orderId}`);
  ...
  drawText(customerSection.left, customerSection.top + 18, 11, input.customerName || 'Customer name', true);
  ...
  wrapText(input.address || 'Address not added', 52).forEach((line, index) => {
  ...
  drawText(orderSection.left, orderSection.top + 18, 10.5, `Lens Type: ${input.lensType}`);
  ...
  drawText(paymentSection.left, paymentSection.top + 22, 10.5, `Payment Mode: ${input.paymentMode}`);
...
function bytesToPdfBinaryString(bytes: Uint8Array) {
  let result = '';
...
async function loadPdfImageFromUri(uri: string) {
  if (typeof document === 'undefined') {
    return null;
  }

  return new Promise<{ width: number; height: number; data: Uint8Array }>((resolve, reject) => {
